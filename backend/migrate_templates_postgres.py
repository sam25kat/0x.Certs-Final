...
async def migrate_templates_to_postgres():
    """Migrate existing template files to PostgreSQL database"""
    ...
    print(f"Migrating templates from {template_dir} to PostgreSQL database...")
    ...
    for filename in template_files:
        try:
            ...
            print(f"Migrating {filename} as '{name}'...")
    ...
    if migrated_count > 0:
        print(f"\nAll templates are now stored in the PostgreSQL database.")
...
async def main():
    ...
    print("Starting template migration to PostgreSQL database...")
    await migrate_templates_to_postgres()
